...
func TestEmptyBitmap(t *testing.T) {
	...
	bm := NewBitmap(a[5:]...)
	a = a[5:]
	if !reflect.DeepEqual(bm.Slice(), []uint64(nil)) {
		t.Fatalf("slice: %v \n bm: %v", a, bm.Slice())
	}
}

func TestSliceRangeCrasher(t *testing.T) {
	a := []uint64{65536}
	bm := NewBitmap(65536)
	start, end := uint64(1), uint64(131072)
	expected := rangeSlice(a, start, end)
	actual := bm.SliceRange(start, end)
	if !reflect.DeepEqual(expected, actual) {
		t.Fatalf("slice range:\n from %v to %v \n in slice: %v and bm: %v \n expected %v and got %v",
			start, end, a, bm.Slice(), expected, actual)
	}
}

func TestCountRangeCrasher(t *testing.T) {
	a := []uint64{65536}
	bm := NewBitmap(65536)
	start, end := uint64(65536), uint64(2)
	expected := countRangeSlice(a, start, end)
	actual := bm.CountRange(start, end)
	if expected != actual {
		t.Fatalf("count range:\n from %v to %v \n in slice: %v and bm: %v \n expected %v and got %v",
			start, end, a, bm.Slice(), expected, actual)
	}
}

func TestShiftCrasher(t *testing.T) {
	a := []uint64{65535, 131073}
	bm := NewBitmap(65535, 131073)
	expected := shiftSlice(a, 1)
	bm1, _ := bm.Shift(1)
	actual := bm1.Slice()
	if !reflect.DeepEqual(expected, actual) {
		t.Fatalf("shift:\n in slice: %v and bm: %v \n expected %v and got %v",
			a, bm.Slice(), expected, actual)
	}
}

func TestDirectRemoveNCrasher(t *testing.T) {
	a := []uint64{1, 34}
	bm := NewBitmap(1, 34)
	bm1 := bm.Slice()
	v1, v2 := uint64(1), uint64(131073)
	expected, ech := removeNFromSlice(a, v1, v2)
	ach := bm.DirectRemoveN(v1, v2)
	actual := bm.Slice()
	if ech != ach || !reflect.DeepEqual(expected, actual) {
		t.Fatalf("directRemoveN:\n removing %v and %v \n in slice: %v and bm: %v \n expected %v and %v changed \n got %v and %v changed",
			v1, v2, a, bm1, expected, ech, actual, ach)
	}
}
